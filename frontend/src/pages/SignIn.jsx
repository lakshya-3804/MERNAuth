...
export default function SignIn() {
  ...
  const handleLogin = async (e) => {
    e.preventDefault();   // prevent data from being reset
    try {
      // send post request to server for authentication as per form data
      const res = await fetch('http://localhost:1234/api/v1/signIn', {
        method: 'POST',
      ...
      dispatch(setIsLoggedIn(true));    // handle isLoggedIn state 
      navigate('/home');
  ...
  const handleGoogleSignIn = async ()=>{
    try {
      ...
      const res = await fetch("http://localhost:1234/api/v1/signIn/google",{
        method : 'POST',
        headers: {'Content-Type': 'application/json'},
        credentials : 'include',
        body: JSON.stringify({
      ...
      <div className="mx-auto mt-[60px] w-full max-w-sm p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-6 md:p-8 dark:bg-gray-800 dark:border-gray-700">
        <form onSubmit={handleLogin} className="space-y-6" action="#">
          ...
          <div>
              ...
              <input type="email" autoComplete="on" value={email} onChange={(e)=>setEmail(e.target.value)} name="email" id="email" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" placeholder="name@company.com" required />
          </div>
          <div>
          ...
          <div className="flex flex-col justify-between items-center gap-[10px]">
            ...
            <button type="button" onClick={handleGoogleSignIn} className=" inline-flex w-full  items-center justify-center rounded-md text-white bg-black/50 hover:bg-black focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">
